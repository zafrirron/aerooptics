...
vData.forEach(function(v) {
    let block = tpl.split("{{id}}").join(v.id)
    ...
    btnHTML += '                <button onclick="switchVertical(\\'' + v.id + '\\')" id="nav-' + v.id + '" class="w-full text-left px-3 py-3 sm:px-4 rounded-lg text-sm font-bold transition-all duration-300 text-slate-400 hover:text-white hover:bg-slate-900 border border-transparent flex items-center justify-center sm:justify-start gap-3">\\n' +
               '                    <i class="fa-solid ' + v.icon + ' ' + v.c_text + ' text-lg w-5 text-center"></i> <span class="hidden sm:inline">' + v.name + '</span>\\n' +
               '                </button>\\n';

    calcJS += '        function calc_' + v.id + '() {\\n' +
...
const switchJSFunctions = '        function switchTab(vertical, tab) {\\n' +
...
const stringifiedKeys = "['optics', 'battery', " + vData.map(function(v){return "'" + v.id + "'"}) + "]";
